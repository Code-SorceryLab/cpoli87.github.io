...
_Code & Sorcery Lab — Ontario Tech University_
...
You will not be penalized for a weak agent if your formulation, analysis, and writeup are sound. You _will_ be penalized for a "working" agent you can't explain, can't reproduce, or that was copied wholesale.
...
- **You must write the game yourself.** Do **not** use an existing Mario environment (e.g. `gym-super-mario-bros`, the original ROM, or a NES emulator) as your environment. Reimplementing the _idea_ of Mario is the point; a generic tile-based platformer of your own is perfectly acceptable and even encouraged.
...
- A clearly documented **observation space** (feature vector _or_ pixels — your call, justify it), **action space** (discrete is fine), and **reward function**.
...
- **Generalization.** Train on a set of levels and evaluate on **held-out or procedurally generated levels** the agent never saw. Report the generalization gap. _(This is the single most research-relevant stretch goal.)_
...
- **Libraries.** Game engines/frameworks (Pygame, SDL, raylib, Godot, a browser canvas, etc.) are fine for the game. RL libraries are fine for the Core agent. **Exception:** the _"PPO from scratch"_ stretch goal must not use an RL library for the algorithm itself.
...
- **MDP formulation** — your observation space, action space, and reward function, and _why_.
...
3. We review it and schedule a **~45-minute interview**. Expect to:
   ...
   - answer questions about _any_ line of code in your repo, and
